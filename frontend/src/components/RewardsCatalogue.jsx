...
import { CATEGORY_META, CATALOGUE, CAT_PERSONAS, personaFor, fmtPts, visualFor, onImgError, rewardImgStyle, visibleTo, grantVouchersFromCart, bumpLeanWeights } from "../lib/catalogue.js";
...
/* ---------------- persist store state across reloads (localStorage only) ----------------
   ...
   writes the bank's Postgres ledger. But plain in-memory React state means every page
   refresh silently resets balances/carts back to the seed values. This makes the DEMO
   STATE survive a reload/localhost-restart, without touching the DB. */
...
function savePersisted(balances, carts) {
  try { localStorage.setItem(STORE_KEY, JSON.stringify({ balances, carts })); } catch { /* quota/private mode */ }
}
...
export function RewardsCatalogue({ theme, initialCategory = null, onExit, onOpenTravel }) {
  ...
  /* +/- quantity stepper — used by the cart panel and the chat bubble's inline
     ...
     same way addToCart is, so the user is told immediately if they can't afford
  ...
  function checkoutCart() {
    ...
    // Redeemed airline/dining vouchers become USABLE assets in the chat
    // (live Duffel flights / food ordering) — and every checkout nudges the
    // dynamic lifestyle weights, mirroring the bank's preference learning.
    grantVouchersFromCart(activeUserId, cart);
    bumpLeanWeights(persona, cart.map((i) => i.category));
  ...
  return (
    <div style={{ position: "relative", height: "100vh", display: "flex", flexDirection: "column",
      ...
      <CredArtBubble persona={persona} balance={balance} cartCount={cart.length}
        cartTotal={cartTotal(cart)} cart={cart} onChangeQty={changeQty}
