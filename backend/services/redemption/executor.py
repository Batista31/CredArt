"""Redemption executor — demo + production with optional OTP-gated bookings."""
...
def _fail(
    ...
    return {
        ...
        "path": provider.path,
        "mode": mode,
        "option_label": candidate.get("label", ""),
        "card_id": card_id,
        "card_name": card_name,
...
async def start_redemption(
    ...
    if row is None:
        return _fail(txn_id, provider, candidate, card_id, card_id, mode, "user does not hold this card", [])
    ...
    if mode == "production" and provider.requires_otp:
        sess = bss.create(
            ...
            merchant=candidate.get("label", "booking"),
            amount=cost,
            card_last4=_last4(row.get("card_number_masked")),
        )
        return {
            "status": "otp_required",
            "transaction_id": txn_id,
...
async def submit_otp(booking_session_id: str, otp: str) -> dict:
    ...
    if sess is None:
        return {
            ...
            "rollback_reason": "unknown booking session",
        }
    if not ok:
    ...
    if provider is None:
        bss.mark(booking_session_id, "failed", error_message="unknown provider")
        return {
            "status": "failed",
            "booking_session_id": booking_session_id,
            "rollback_reason": "unknown provider",
        }
    ...
    result = await _finalize(uuid4().hex, sess["user_id"], candidate, provider, {}, "production", row)
    bss.mark(
        booking_session_id,
        result["status"],
...
async def _finalize(
    ...
    try:
        ...
        await db.execute(
            ...
            user_card_id,
            txn_id,
            candidate.get("kind", "redemption"),
            stored_label,
            cost,
            candidate.get("best_use_case"),
        )
        ...
        if not result.ok:
            await db.execute(
                ...
                txn_id,
                (result.error or "provider booking failed")[:500],
            )
            return _fail(
                txn_id,
                provider,
                ...
                card_name,
                mode,
                result.error or "provider booking failed",
                result.steps,
            )
            ...
            async with conn.transaction():
                ...
                if new_balance is None:
                    await conn.execute(
                        "UPDATE redemption_history SET status='failed', rollback_reason=$2 WHERE transaction_id=$1",
                        txn_id,
                        "balance changed during booking",
                    )
                    return _fail(
                        txn_id,
                        provider,
