...
async def add_address(user_id: str, payload: dict) -> dict:
    if payload.get("is_default"):
        await db.execute("UPDATE user_saved_addresses SET is_default = FALSE WHERE user_id = $1", user_id)
    return await db.fetchrow(
        """
        INSERT INTO user_saved_addresses
            (user_id, label, line1, line2, city, state, postal_code, country, is_default)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
        RETURNING id, label, line1, line2, city, state, postal_code, country, is_default
        """,
        user_id,
        payload["label"],
        payload["line1"],
